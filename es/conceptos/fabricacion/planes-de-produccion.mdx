...
| --- | --- |
| **Básico** | Busca rápidamente una secuencia factible con una heurística. Si no logra completar la secuencia, el plan queda en **Fallido**. |
...
El diálogo **Resolver** selecciona **Básico** inicialmente. Si integras mediante la API y omites `mode` al crear y resolver un plan o al llamar a la operación de resolución, el servidor utiliza **Avanzado**.
...
| **Fallido** | El cálculo no terminó correctamente. En modo básico, también indica que la heurística no pudo completar la secuencia. |
